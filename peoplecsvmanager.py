...
class PeopleCsvManager:
    def __init__(
        self, csv_file="personer.csv", people_to_ignore="personer_ignorer.csv"
    ) -> None:
    ...
    def getPersonData(self, person_outlook_name):
        ...
        for person in self.__people:
            ...
            self.logger.debug(
                f"Sammenligner OUTLOOK NAVN {person_outlook_name} med Registernavn {outlook_name}"
            )
        ...
        for person in self.__people_to_ignore:
            if person["outlook_name"] == person_outlook_name:
                self.logger.debug("FOUND and should be IGNORED")
    ...
    def __readFile_ignore(self, csv_file="personer_ignorer.csv"):
        ...
        try:
            ...
            self.logger.warning(
                f"CSV filen '{csv_file}'' blev ikke fundet. Prøver at oprette den, og genkøre sig køre igen."
            )
            self.logger.debug(e)
        ...
        try:
            with open(csv_file) as csv_file:
                ...
                for row in csv_reader:
                    ...
                    self.logger.debug(
                        f"\t{row['Outlook navn']} har ALIAS {row['AULA navn']} ."
                    )
                    line_count += 1
