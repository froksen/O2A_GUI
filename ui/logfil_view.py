        ...
        cls._records.append(
            {
                ...
                "lvl": {10: "debug", 20: "info", 30: "warn", 40: "err", 50: "err"}.get(
                    record.levelno, "info"
                ),
...
class LogfilView(tk.Frame):
    ...
    def _build(self):
        ...
        tk.Label(
            title_row,
            text="Alt output fra O2A",
            bg=BG,
            fg=TEXT,
            font=self._fonts["display_m"],
        ).pack(side="left")
        ...
        tk.Label(
            self,
            text=(
                "Komplet rå-output fra alle kørsler. "
        ...
        tk.Label(
            retention_row,
            text="Logfilen slettes automatisk efter ",
            bg=BG,
            fg=DIM,
            font=self._fonts["small"],
        ).pack(side="left")
        tk.Label(
            retention_row,
            text=str(LOG_RETENTION_DAYS),
            bg=BG,
            fg=TEXT,
            font=self._fonts["body"],
        ).pack(side="left")
        tk.Label(
            retention_row, text=" dage.", bg=BG, fg=DIM, font=self._fonts["small"]
        ).pack(side="left")
        ...
        for lvl_id in self._levels:
            self.txt.tag_configure(
                f"lvl_{lvl_id}", foreground=STATUS_COLORS.get(lvl_id, TEXT)
            )
            self.txt.tag_configure(f"elide_{lvl_id}", elide=False)

        self.txt.tag_configure("ts", foreground=FAINT)
        self.txt.tag_configure("lvlcol", foreground=FAINT, font=self._fonts["mono_sm"])
        self.txt.tag_configure(
            "run_hdr",
            foreground=DIM,
            font=self._fonts["eyebrow"],
        ...
        self.txt.tag_configure(
            "err_band", background="#FBEEEA", lmargin1=32, lmargin2=32
        )
    ...
    def _make_chip(self, parent, lvl_id, label):
        ...
        lbl = tk.Label(c, text=label, bg=SUBTLE, fg=TEXT, font=self._fonts["small"])
    ...
    def _update_chip_appearance(self, chip, lvl_id):
        ...
        chip.config(
            bg=bg, highlightthickness=1, highlightbackground=LINE if on else SUBTLE
        )
