...
class LoginErrorDialog:
    ...
    def __init__(self, parent, fonts, on_fix_credentials=None):
        ...
        tk.Label(
            self.top,
            text="Kunne ikke logge ind på Aula",
            bg=PANEL,
            fg=TEXT,
            font=fonts["display_s"],
            justify="left",
        ...
        tk.Label(
            self.top,
            text=(
                "Synkroniseringen blev afbrudt, da det ikke lykkedes\n"
        ...
        for cause in [
            ...
            tk.Label(row, text="•", bg=PANEL, fg=DIM, font=fonts["body"]).pack(
                side="left", padx=(4, 8)
            )
            tk.Label(row, text=cause, bg=PANEL, fg=DIM, font=fonts["body"]).pack(
                side="left"
            )
