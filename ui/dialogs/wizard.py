...
from theme import PANEL, SUBTLE, LINE, TEXT, DIM, ACCENT, OK, ERR
...
class FirstRunWizard:
    ...
    def _build_step_welcome(self):
        ...
        tk.Label(
            f, text="OPSÆTNING", bg=PANEL, fg=DIM, font=self._fonts["eyebrow"]
        ).pack(anchor="w", padx=26, pady=(22, 2))
        tk.Label(
            f,
            text="Velkommen til Outlook2Aula",
            bg=PANEL,
            fg=TEXT,
        ...
        tk.Label(
            f,
            text=(
                "Dette opsætningsassistent hjælper dig med at konfigurere "
    ...
    def _build_step_aula_login(self):
        ...
        tk.Label(
            f, text="TRIN 1 AF 3", bg=PANEL, fg=DIM, font=self._fonts["eyebrow"]
        ).pack(anchor="w", padx=26, pady=(22, 2))
        tk.Label(
            f, text="Aula-login", bg=PANEL, fg=TEXT, font=self._fonts["display_s"]
        ).pack(anchor="w", padx=26)
        ...
        tk.Label(
            grid, text="Brugernavn", bg=PANEL, fg=TEXT, font=self._fonts["body_b"]
        ...
        tk.Label(
            grid, text="Kodeord", bg=PANEL, fg=TEXT, font=self._fonts["body_b"]
        ).grid(row=1, column=0, sticky="w", pady=4)
        self._wiz_password = tk.Entry(
            grid, width=28, show="*", relief="solid", borderwidth=1
        )
        self._wiz_password.grid(row=1, column=1, padx=(12, 0), pady=4, ipady=3)

        self._login_status = tk.Label(
            f, text="", bg=PANEL, fg=DIM, font=self._fonts["small"]
        )
    ...
    def _build_step_categories(self):
        ...
        tk.Label(
            f, text="TRIN 2 AF 3", bg=PANEL, fg=DIM, font=self._fonts["eyebrow"]
        ).pack(anchor="w", padx=26, pady=(22, 2))
        tk.Label(
            f,
            text="Outlook-kategorier",
            bg=PANEL,
            fg=TEXT,
            font=self._fonts["display_s"],
        ).pack(anchor="w", padx=26)
        ...
        tk.Label(
            f,
            text=(
                "Outlook2Aula anvender to kategorier i Outlook til at identificere "
    ...
    def _build_step_ready(self):
        ...
        tk.Label(
            f, text="TRIN 3 AF 3", bg=PANEL, fg=DIM, font=self._fonts["eyebrow"]
        ).pack(anchor="w", padx=26, pady=(22, 2))
        tk.Label(
            f, text="Klar!", bg=PANEL, fg=TEXT, font=self._fonts["display_s"]
        ).pack(anchor="w", padx=26)

        tk.Label(
            f,
            text=(
                "Opsætningen er nu gennemført.\n\n"
    ...
    def _go_next(self):
        ...
        if self._step == 1:
            ...
            try:
                ...
                self._login_status.config(text="✓ Login-oplysninger gemt.", fg=OK)
            except Exception as e:
                self._login_status.config(text=f"Fejl ved gemning: {e}", fg=ERR)
                return
            ...
            try:
                ...
                self._cat_status.config(text="✓ Kategorier oprettet i Outlook.", fg=OK)
            except Exception as e:
