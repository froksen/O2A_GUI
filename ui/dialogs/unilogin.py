...
from theme import PANEL, LINE, TEXT, DIM, ACCENT, SUBTLE
...
class UniloginDialog:
    ...
    def __init__(self, parent: tk.Misc, fonts=None):
        ...
        px = (
            parent.winfo_rootx() + (parent.winfo_width() - self._dlg.winfo_width()) // 2
        )
        py = (
            parent.winfo_rooty()
            + (parent.winfo_height() - self._dlg.winfo_height()) // 2
        )
    ...
    def _build_ui(self):
        ...
        # Overskrift
        tk.Label(
            d, text="LOGIN-OPSÆTNING", bg=PANEL, fg=DIM, font=("Segoe UI", 8, "bold")
        ).pack(anchor="w", padx=26, pady=(22, 2))

        tk.Label(
            d, text="Log ind på Aula", bg=PANEL, fg=TEXT, font=("Georgia", 15)
        ...
        tk.Label(
            grid, text="Loginmetode", bg=PANEL, fg=TEXT, font=("Segoe UI", 9, "bold")
        ...
        # Brugernavn
        tk.Label(
            grid, text="Brugernavn", bg=PANEL, fg=TEXT, font=("Segoe UI", 9, "bold")
        ...
        # Kodeord
        tk.Label(
            grid, text="Kodeord", bg=PANEL, fg=TEXT, font=("Segoe UI", 9, "bold")
        ).grid(row=2, column=0, sticky="w", pady=4)
        self.password = tk.Entry(
            grid, width=30, show="*", relief="solid", borderwidth=1
        )
